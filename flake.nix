...
#+BEGIN_SRC nix
{
      ...
      overlays = builtins.map (o: o // { value = lib.fix o.__functor; }) [{
        ...
        __functor = overlaysBase: final: prev: let
          overlaySets = lib.mapAttrs' (ident: overlay: {
            ...
            value = import final.pkgs.path {
              inherit (prev) system config;
              overlays = prev.overlays ++ [rec {
                value = final: prev: {
                  inherit overlay;
                  pkgsParent = prev;
                } // overlay final prev;
        ...
        __functor = _: final: prev: with prev; {
          inherit (withGuixFlake) guix;
          inherit (withEmacsFlake.withEmacs.withEmacs-weechat) emacsPgtkGcc emacsPgtkGccPackages;
          ...
          inherit (withEmacsFlake.withEmacs.withEmacs-weechat) emacsPackages;
  ...
  in {
     ...
     #(flakeOverlay { flake = "lg400"; name = "delta/system-400-link"; })
      ...
      (listToAttrs (map
        (name: {
          ...
          value = final: prev: builtins.trace "reading overlay ${name}" (import (./overlays + "/${name}") inputs final prev);
        })
        (builtins.filter
    ...
    packages = forAllSystems ({ pkgs, ... }: let
      pkgs' = pkgs.withIni2json // builtins.trace "pkgs.pleroma: see instead nixpkgs#103138" {
