(use-modules (gnu)
             ...
             (srfi srfi-1))
(use-service-modules desktop xorg sddm shepherd
                     ssh networking nix vpn)
(use-package-modules certs gnome vim admin
                     chromium file ssh rust-apps
                     tmux dvtm version-control shells
                     networking compression cpio vpn
                     python web-browsers curl screen
                     emacs emacs-xyz xorg wm ipfs
                     messaging irc web rsync gnupg)
