{ lib, fetchFromGitHub, coreutils, llvmPackages_latest, gcc, makeWrapper, libtool, gdb
, runCommandNoCCLocal, runCommandCC, linkFarm, writeText
, libsodium, openssl, file, boost, gmp, rocksdb, jemalloc, zlib, riot-web
...
let
  ...
  rev = "a8870908a5735f5f5349bc24d2feaf9e902af7ed";
  hash = "sha256-rqU5KdIgoPXixWXmr4GBnjR7ljPFG8zbE5zrlv9pwsg=";
  ...
  source = let
    ...
    repo = lib.cleanSourceWith { filter = srcFilter; inherit src; };
...
in stdenv.mkDerivation rec {
  ...
  includes = stdenv.mkDerivation rec {
    ...
    configHeader = writeText "config.h" ''
  ...
  installPhase = let
    ...
    ircd = ircdLD [
      ...
      "${ircdUnitCXX "db_write_thread.cc"        "db_write_thread.lo"        "-I${rocksdb-pinned.out} -I${rocksdb-pinned.out}/include"}"
      "${ircdUnitCXX "db_crc32.cc"        "db_crc32.lo"        "-I${rocksdb-pinned.out} -I${rocksdb-pinned.out}/include"}"
