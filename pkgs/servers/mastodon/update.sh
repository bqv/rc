...
#!nix-shell -i bash -p stdenv nix-prefetch-github bundix coreutils jq curl nix git gawk gnused
set -ve
...
#sed 's/}:/, ...}:/' -i "$TARGET_DIR/yarn.nix"
sed 's/undefined/rz4nz7r7sx7bv0p0azs2dffnndmals0p/' -i "$TARGET_DIR/yarn.nix"
