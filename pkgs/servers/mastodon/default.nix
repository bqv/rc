{ nodejs-slim, fetchgit, fetchFromGitHub, bundlerEnv,
  stdenv, yarn, callPackage, imagemagick, ffmpeg, file, ... }:

with callPackage ./yarn2nix {};
let
  ...
  mastodon-js-modules = mkYarnPackage {
    name = "mastodon-modules-${version}";
    pkg = "mastodon-modules";
    yarnNix = ./yarn.nix;
    packageJSON = ./package.json;
    extraBuildInputs = [ yarn2nix ];
    inherit src;
    inherit version;
  };

  mastodon-assets = stdenv.mkDerivation {
    name = "mastodon-assets-${version}";
