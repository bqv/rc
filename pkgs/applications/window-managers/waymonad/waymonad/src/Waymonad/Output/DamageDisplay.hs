...
import Graphics.Wayland.WlRoots.Output
    ( getOutputDamage , isOutputEnabled, outputTransformedResolution
    , setOutputDamage, getOutputTransform, getTransMatrix, outputGetBackend
    )
...
damageDisplay depth secs out@Output {outputRoots = output, outputLayout = layers} = do
    ...
    needsSwap <- liftIO . pixmanRegionNotEmpty $ getOutputDamage output
