(define-module (gnu s6-services)
  ...
  #:export (s6-service-type
            s6-profile-service-type
            s6-environment-variables-service-type
            s6-files-service-type
            s6-run-on-first-login-service-type
            s6-activation-service-type
            s6-run-on-change-service-type
            s6-provenance-service-type

            fold-s6-service-types)
...
;;;
;;; s6-service-type is a root of s6 services DAG.
;;;
;;; s6-profile-service-type is almost the same as profile-service-type, at least
;;; for now.
;;;
;;; s6-environment-variables-service-type generates a @file{setup-environment}
...
;;; automatically add code for sourcing this file, if person do not use those s6
;;; services they have to source this script manually in their's shell *profile
;;; file (details described in the manual).
;;;
;;; s6-files-service-type is similar to etc-service-type, but doesn't extend
;;; s6-activation, because deploy mechanism for config files is pluggable and
;;; can be different for different s6 environments: The default one is called
...
;;; possible to implement alternative approaches like read-only s6 from Julien's
;;; guix-s6-manager.
;;;
;;; s6-run-on-first-login-service-type provides an @file{on-first-login} guile
...
;;;
;;; s6-activation-service-type provides an @file{activate} guile script, which
...
;;;
;;; - Run all activation gexps provided by other s6 services.
;;;
;;; s6-run-on-change-service-type allows to trigger actions during
;;; activation if file or directory specified by pattern is changed.
;;;
;;; Code:


(define (s6-derivation entries mextensions)
  "Return as a monadic value the derivation of the 's6'
directory containing the given entries."
  (mlet %store-monad ((extensions (mapm/accumulate-builds identity
                                                          mextensions)))
    (lower-object
     (file-union "s6" (append entries (concatenate extensions))))))

(define s6-service-type
  ;; This is the ultimate service type, the root of the s6 service
  ;; DAG.  The service of this type is extended by monadic name/item
  ;; pairs.  These items end up in the "s6-environment directory" as
  ;; returned by 's6-environment-derivation'.
  (service-type (name 's6)
                (extensions '())
                (compose identity)
                (extend s6-derivation)
                (default-value '())
                (description
                 "Build the s6 environment top-level directory,
which in turn refers to everything the s6 environment needs: its
...
;; place it in s6-profile-service-type to affect all profile
;; packages and prevent conflicts, when other packages relies on
;; non-transformed version of package.
(define s6-profile-service-type
  (service-type (name 's6-profile)
                (extensions
                 (list (service-extension s6-service-type
                                          packages->profile-entry)))
                (compose concatenate)
                (extend append)
                (description
                 "This is the @dfn{s6 profile} and can be found in
@file{~/.guix-s6/profile}.  It contains packages and
configuration files that the user has declared in their
@code{s6-environment} record.")))

(define (environment-variables->setup-environment-script vars)
  "Return a file that can be sourced by a POSIX compliant shell which
initializes the environment.  The file will source the s6
...
(define s6-environment-variables-service-type
  (service-type (name 's6-environment-variables)
                (extensions
...
(define (files-entry files)
  "Return an entry for the @file{~/.guix-s6/files}
directory containing FILES."
  (with-monad %store-monad
    (return `(("files" ,(files->files-directory files))))))

(define s6-files-service-type
  (service-type (name 's6-files)
                (extensions
                 (list (service-extension s6-service-type
                                          files-entry)))
                (compose concatenate)
...
will be put in @file{~/.guix-s6/files}.")))
...
in the s6 environment directory."
  (mlet %store-monad ((on-first-login m-on-first-login))
        (return `(("on-first-login" ,on-first-login)))))

(define s6-run-on-first-login-service-type
  (service-type (name 's6-run-on-first-login)
                (extensions
                 (list (service-extension
                        s6-service-type
...
(define (compute-activation-script init-gexp gexps)
  (gexp->script
   ...
   #~(let* ((he-init-file (lambda (he) (string-append he "/setup-environment")))
            (he-path (string-append (getenv "HOME") "/.guix-s6"))
            (new-s6-env (getenv "GUIX_NEW_HOME"))
            (new-s6 (or new-s6-env
                          ;; Path of the activation file if called interactively
                          (dirname (car (command-line)))))
            (old-s6-env (getenv "GUIX_OLD_HOME"))
            (old-s6 (or old-s6-env
                          (if (file-exists? (he-init-file he-path))
                              (readlink he-path)
                              #f))))
       (if (file-exists? (he-init-file new-s6))
           (let* ((port   ((@ (ice-9 popen) open-input-pipe)
             ...
             (setenv "GUIX_NEW_HOME" new-s6)
             (setenv "GUIX_OLD_HOME" old-s6)

             #$@gexps

             ;; Do not unset env variable if it was set outside.
             (unless new-s6-env (setenv "GUIX_NEW_HOME" #f))
             (unless old-s6-env (setenv "GUIX_OLD_HOME" #f)))
           (format #t "\
Activation script was either called or loaded by file from this direcotry:
~a
It doesn't seem that s6 environment is somewhere around.
Make sure that you call ./activate by symlink from -s6 store item.\n"
                   new-s6)))))
...
(define s6-activation-service-type
  (service-type (name 's6-activation)
                (extensions
                 (list (service-extension
                        s6-service-type
...
generation of s6 environment and update the state of the s6
...
(define s6-run-on-change-service-type
  (service-type (name 's6-run-on-change)
                (extensions
                 (list (service-extension
                        s6-activation-service-type
                        identity)))
...
(define s6-provenance-service-type
  (service-type
   (name 's6-provenance)
   (extensions
    (list (service-extension
   ...
   (description "\
Store provenance information about the s6 environment in the s6
environment itself: the channels used when building the s6
environment, and its configuration file, when available.")))

(define sexp->s6-provenance sexp->system-provenance)
(define s6-provenance system-provenance)
...
(define %guix-s6-root-directory
  ;; Absolute file name of the module hierarchy.
  (parent-directory (dirname (search-path %load-path "gnu/s6-services.scm"))))

(define %service-type-path
  ;; Search path for service types.
  (make-parameter `((,%guix-s6-root-directory . "gnu/s6-services"))))

(define (all-s6-service-modules)
  "Return the default set of s6-service modules."
  (cons (resolve-interface '(gnu s6-services))
        (all-modules (%service-type-path)
                     #:warn warn-about-load-error)))

(define* (fold-s6-service-types proc seed)
  (fold-service-types proc seed (all-s6-service-modules)))
